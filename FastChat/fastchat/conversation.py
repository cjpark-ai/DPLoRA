# Modified by the DPLoRA authors from upstream LMSYS FastChat (commit 587d5cf):
# added the conversation templates `alpaca_dplora_llama` and `alpaca_dplora_qwen`
# (registered below). Licensed under Apache-2.0 (see ../LICENSE); this change
# notice is provided per Apache-2.0 section 4(b).
"""
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style == SeparatorStyle.ADD_COLON_SPACE_TWO:
            seps = [self.sep, self.sep2]
            ret = system_prompt + seps[0]
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role + ": "  # trailing space to match training template
            return ret
...
# DPLoRA Alpaca template for Llama-3 models
register_conv_template(
    Conversation(
        name="alpaca_dplora_llama",
        system_message="Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n",
        roles=("### Instruction:\n", "### Response:"),
...
# DPLoRA Alpaca template for Qwen models
register_conv_template(
    Conversation(
        name="alpaca_dplora_qwen",
        system_message="Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n",
        roles=("### Instruction:\n", "### Response:"),
        sep_style=SeparatorStyle.NO_COLON_TWO,
        sep="\n\n",
