...
class DPLoRAAdapter(BaseModelAdapter):
    """The model adapter for DPLoRA-trained Alpaca models (Llama-3.x / Qwen).

    DPLoRA checkpoints live under "DPLoRA_*" paths that also contain the
    substring "alpaca", which would otherwise select AlpacaAdapter
    (use_fast=False, intended for the original sentencepiece-based Stanford
    Alpaca on LLaMA-1). DPLoRA models are Llama-3.x / Qwen and ship only a fast
    tokenizer, so use_fast=False yields a broken tokenizer. Keep the
    BaseModelAdapter behavior (use_fast=True, the HF default for Llama-3) and
    match on "dplora" so it wins over AlpacaAdapter (registered just before it).
    """

    use_fast_tokenizer = True

    def match(self, model_path: str):
        return "dplora" in model_path.lower()
...
register_model_adapter(DPLoRAAdapter)
